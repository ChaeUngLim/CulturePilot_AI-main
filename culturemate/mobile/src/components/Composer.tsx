...
export function Composer({
  ...
  const onKeyPress = (e: NativeSyntheticEvent<TextInputKeyPressEventData>) => {
    ...
    // 한글 IME 조합 중의 Enter 는 '조합 확정'이지 '전송'이 아니다.
    // react-native-web 은 자기 내부 onSubmitEditing 앞에서만 isComposing 을 보고,
    // onKeyPress 는 그보다 먼저 무조건 호출한다 — 그래서 여기서 직접 걸러야 한다.
    // 안 걸르면 마지막 글자가 조합 중인 채로 전송되거나(글자 누락),
    // 확정 Enter 와 전송 Enter 가 겹쳐 같은 문장이 두 번 나간다.
    // keyCode 229 는 IME 가 키를 처리 중임을 뜻하는 표준 신호다(W3C uievents).
