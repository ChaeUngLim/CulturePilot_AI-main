...
**규모**: 백엔드 Python 11,024줄 / 모바일 TS 6,534줄 / 테스트 166개.
...
[6] validation (5종 ∥)             validation.py   이슈 판정
...
│   ├── router/              874  UR-02  분류 + 규칙 파서 + 좌표 확정 (6모듈)
│   ├── nodes.py             278  조율 노드 6개 (merge/hitl/finalize/persist/compose)
│   ├── state.py             205  State 정의 + 서브그래프 입출력 스키마
│   ├── reducers.py          135  MERGE_BY_ID · merge_candidates · replace_list
│   ├── budget.py             59  응답 시간 예산 (타임아웃과 다르다)
│   ├── serde.py                  체크포인트 직렬화 허용 타입
│   └── subgraphs/
│       ├── itinerary/     1,525  UR-05·06·07·08 (9모듈)
│       ├── discovery.py     534  UR-03·04
│       ├── validation.py    345  검증 6종 → triage → 카드
...
│   ├── writer.py            161  UR-10  경험 요약 + 임베딩
│   └── profile.py           127  취향 프로필 재구성
├── llm/provider.py          233  역할별 모델 배분 (router/planner/writer/fast)
...
├── api/
│   ├── client.ts            506  SSE + sync 폴백 + 2단계 호출
│   ├── mock.ts              410  오프라인 개발용
│   └── types.ts             227  서버 계약
└── hooks/useCultureMate.ts  254  상태 관리 · fillRoutes()
```
