...
**규모**: 백엔드 Python **11,613줄**(56파일) / 모바일 TS **6,542줄**(`src` 4,042 + `app` 2,500) / 테스트 **194개**.
...
| **UR-41** | 화면 명세(참조 화면 23장) | ✅ | `app/ui.py` · 근거는 [UI.md](UI.md) |
| **UR-42** | 비회원/회원 시작 · 로그인 게이트 | ⬜ | 명세만 있다. **인증 0개** — `user_id` 는 클라이언트 값 |
| **UR-43** | 비회원 기록 이어받기 | ⬜ | 익명 uuid → 계정 uuid 이관 경로 없음 (테이블 8개) |
...
├── ui.py                    956  UR-41  화면 명세 (그래프 밖 · 렌더는 mobile/)
├── graph/
