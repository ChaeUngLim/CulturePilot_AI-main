...
| 리듀서 | 쓰는 곳 | 규칙 |
|---|---|---|
| `MERGE_BY_ID` | archive_hits, evidence, edit_signals, gaps … | 같은 id 는 나중 것으로 갱신 |
...
| `merge_candidates` | candidates | 같은 장소는 **정보가 더 풍부한 쪽**을 남긴다 |
| `merge_dict` | transport_mix 등 | 얕은 병합 |
...
다섯 검사는 각자 `Issue` 만 쌓고 **판단은 하지 않는다.** 검사마다 '이건 사용자에게
