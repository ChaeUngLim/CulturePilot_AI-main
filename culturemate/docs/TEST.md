...
| 항목 | 결과 |
|---|:--:|
| 자동 테스트 (pytest) | ✅ **127 passed, 1 skipped** <br/>_(2026-08-13 실행 로그 기준 · 현재 194개 → [PROGRESS.md](PROGRESS.md))_ |
| 정적 검사 (ruff) | ✅ **All checks passed** |
| 클라이언트 타입 (tsc) | ✅ 통과 |
| 외부 API 연동 | ✅ **11 / 11** _(프로브 기준 — [SETUP.md §4](SETUP.md))_ |
| 지역별 시나리오 | ✅ **7 / 7 응답** |
...
| **UR-28** | **캘린더로 일정 확인** ★핵심 | ✅ | `GET /plans/{user_id}?from=&to=`(요약) + `GET /plans/detail/{plan_id}` + `(tabs)/calendar.tsx` (2026-08-17). 저장은 원래 되고 있었고 읽는 쪽이 붙었다 |
| **UR-40** | 과거 불편의 선제 경고 | ✅ | `validation.check_friction` 이 `Issue(kind="past_friction")` 를 생산한다 — 검증 5→6종, 계약 테스트도 `six_checks` 로 갱신 (2026-08-17) |
| **FR-25** | 카드 스와이프 온보딩 | ✅ | UR-01과 같은 사안 |
| **FR-26** | 영화 상영 시간표 | ⬜ | 상영 시간표 API가 유료라 보류 |
| **FR-27·28** | 캘린더 조회 API · 화면 | ✅ | UR-28과 같은 사안 (2026-08-17) |
| **FR-29** | 과거 불편 → 경고 승격 | ✅ | UR-40과 같은 사안 (2026-08-17) |
...
|---|---|---|:--:|:--:|
| **UR-28** | **캘린더로 일정 확인** ★핵심 | 4.3 선순환의 «다시 열기» | ✅ (2026-08-17) | — |
| **UR-40** | 과거 불편의 선제 경고 복원 | 2.1-② 선제적 알림과 대안 | ✅ (2026-08-17) | — |
...
> UR-28 · UR-40 은 **2026-08-17 에 완료됐다.** 아래 두 문단은 완료 전의 우선순위
> 판단 기록이다 — 왜 이 순서로 갚았는지의 근거로 남긴다.
...
| 대상 | 조치 |
|---|---|
| ~~UR-28~~ | **완료 2026-08-17** — `repo.list_plans(user_id, frm, to)` → `GET /plans/{user_id}` · `GET /plans/detail/{plan_id}` → `(tabs)/calendar.tsx`. Timeline·NaverMap 재사용. 목록에 `payload` 전문 금지 |
| ~~UR-40~~ | **완료 2026-08-17** — `validation.check_friction` 추가(검증 5→6종). `archive_hits` 의 friction 태그가 이번 일정 항목과 겹치면 `Issue(kind="past_friction")`. 문서·계약 테스트 동시 수정 |
...
```
판교역 아침 7시에 출발해서 강남구에서 문화공연 일정 2~3개 추천 후 21시에 홍대역으로 도착하는 일정 만들어줘
9월에 추천하는 서울시에서 문화공연 일정 만들어줘
10월 주말에 성남시에서 문화공연 일정 만들어줘
9월~10월 인천시에서 가장 유명한 행사 및 축제 일정 만들어줘
충청남도 유명한 특산물 축제 9월달 주말 가능한 일정 만들어줘
부산시에서  8월 주말에 가능한 축제 및 맛집 일정 만들어줘
해운대에서 9~12월 주말에 유명한 축제 및 맛집 일정 만들어줘
```
