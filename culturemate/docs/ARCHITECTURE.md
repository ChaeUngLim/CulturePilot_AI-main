...
> v0.2 (MVP) — 원본 기획서 `아키텍처_2.docx` · 서비스 기획안 `서비스 기획안 모비딕_남경.pdf`(2026-08-13)
>
...
> **모델 배치**: 라우팅·요약·태깅 같은 대량 저지연 작업은 8B급(`MODEL_FAST`), 일정 추론·관련성 판정은 70B급(`MODEL_PLANNER`)으로 분리한다. 전량 대형 모델을 쓰면 일정 1건당 LLM 호출이 10회를 넘어 지연이 사용자 체감 한계를 넘는다.
...
class CultureMateState(TypedDict, total=False):
    ...
    request_type: RequestType          # 라우팅 결과
    flags: PlanFlags                   # 실행 계획
    conditions: TripConditions         # 구조화된 조건
...
`candidates`만 예외적으로 `merge_candidates`를 쓴다. 같은 장소가 공공 API·웹·지도에서 서로 다른 필드 조합으로 오기 때문에, canonical place 기준으로 묶고 **빈 필드는 채우고 점수는 max**로 합친다.

**검증 결과(`issues`·`advisories`)만 누적하지 않고 교체한다**(`replace_list`). 검증은 매 라운드 현재 일정을 처음부터 다시 보므로, 지난 라운드의 이슈는 이미 해결됐거나 그 장소가 일정에서 빠졌을 수 있다. 누적하면 그것들이 영원히 남는다 — 실제로 재계획을 돌수록 카드가 불어나 **이슈 6건이 카드 25장**이 됐고, 일정에서 사라진 장소가 `일정 확인 필요`라는 이름 없는 카드로 계속 떠 있었다. 빈 리스트는 무시한다(검증을 타지 않은 경로가 기존 카드를 지우면 안 된다).
...
#  ArchiveOutput : archive_hits, edit_signals, taste_profile, advisories, evidence, trace
```
...
| **UR-40** | 과거 불편의 선제 경고 복원 | ⬜ | `validation.check_friction` 을 더해 검증 5→6종. 카드·스키마는 이미 있다 |
...
- **UR-40 은 새 노드를 요구한다.** 검증이 5종에서 6종이 되고
  `tests/test_docs_contract.py::test_validation_runs_five_checks` 가 먼저 깨진다.
  §8.1 과 STRUCTURE §6 을 같은 커밋에서 고쳐야 한다.
...
| **선순환 단절** (일정을 다시 못 봄) | 기록이 안 쌓여 다음 바퀴가 첫 사용자와 같아짐 | **UR-28 캘린더** — 지난 일정을 열고 «기록 남기기»로 잇는다. 지금은 이 고리가 비어 있다 |
| **경고가 만들어지지 않음** | 기획안의 핵심 화면(선제적 알림)이 도달 불가 | **UR-40** — `past_friction` 생산자 복원. 소비자(카드·옵션)는 이미 있다 |
...
│         SETUP, PROGRESS, TEST, TEST_FUNCTIONAL}.md, diagrams/*.mmd
...
```
$ pytest -q                    # 서버
128 passed, 1 skipped          # skip: interrupt 왕복 테스트 — 그 입력에서 확인 카드가
