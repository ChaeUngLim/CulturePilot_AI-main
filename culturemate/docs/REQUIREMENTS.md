...
**만든 일정을 다시 여는 수단이 없다.** 지금은 스레드가 살아 있는 동안에만 일정이 보이고,
앱을 닫으면 그날의 일정으로 돌아갈 길이 없다. 기획안 4.3의 선순환
(탐색 → 일정 → 동선 → **기록** → 분석)은 «다시 열기»가 있어야 04에 도달한다.
...
| 11 | 주변 장소 | NAVER 검색 API | 무료 | 식당·카페 채우기 비어 있음 | Food |
