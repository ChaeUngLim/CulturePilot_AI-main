...
| **UR-41** | ⓧ | **화면 명세(참조 화면 23장)** | ✅ | `app/ui.py` — 흐름 4 · 화면 17 · 단계 25 · 선택지 95. `validate()` 가 `TripConditions` Literal·`KIND_GROUPS`·카탈로그 카테고리와 대조한다. 근거는 [UI.md](UI.md) |
| **UR-42** | ⓧ | **비회원/회원 시작 · 로그인 게이트** | ⬜ | 명세만 있다(`FLOW_AUTH` · `GATES` 3건). **인증이 없다** — 엔드포인트 23개 중 인증 0개 · `users` 에 자격증명 컬럼 없음 · `user_id` 를 클라이언트 값 그대로 신뢰. 모바일은 빌드 상수라 설치본 전부가 같은 사용자다 |
| **UR-43** | ⓧ① | **비회원 기록 이어받기** | ⬜ | 로그인 시 익명 uuid → 계정 uuid 이관 경로가 없다. `user_id` 를 가진 테이블 8개(`preference_cards`·`plans`·`plan_edits`·`visits`·`experience_embeddings`·`taste_profiles`·`hitl_decisions`·`user_collections`)를 한 트랜잭션에서 옮겨야 한다. 안 옮기면 로그인하는 순간 온보딩 결과가 사라진다 |
